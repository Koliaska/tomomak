...
from tomomak.util.geometry3d_basic import rotation_matrix_from_vectors
...
def get_obj(vertices, faces=None):
...
def cell_volumes(mesh, index=(0, 1, 2)):
    ...
    if isinstance(index, int):
        index = [index]
    check_spatial(mesh, index)
    trimesh_list = get_grid(mesh, index)
...
def transform_matrix(p1, p2, shift):
